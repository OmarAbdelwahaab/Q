...
- **Word-by-Word Subtitles & RTL BiDi Alignment (`pipeline/render/subtitles.py`)**:
  ...
  - **BiDi / RTL Shaping Architecture**: Addressed libass upstream limitation (issue #406) where standard ASS `{\k}` tags split Arabic lines into separate LTR layout chunks, flipping word order. Subtitle generation emits timed dialogue events with inline color overrides (`{\c&H0037AFD4&}` for active word, `{\c&H00FFFFFF&}` for completed words, and `{\c&H90707070&}` for upcoming words). This preserves continuous Arabic HarfBuzz text shaping, natural Right-to-Left reading order, and clear visual contrast.
