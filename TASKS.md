...
- [x] **Fixed and confirmed: idempotency check-then-act race resolved.** Implemented atomic `claim_execution(message_id, stage, force)` in `PipelineStateRepository` using transactional locks (`BEGIN IMMEDIATE` in SQLite, `BEGIN` in Postgres). Before stage execution starts, `runner.py` acquires an atomic execution claim (`status='processing'`). If another worker is currently processing the item, it exits cleanly with `skipped_active_execution` (exit code 0); if already published, it skips with `skipped_already_published` without re-running any upstream stages. A `--force` CLI flag was also added to allow intentional manual re-runs.
- [x] **Fixed and confirmed: scheduler rate-limit race resolved.** Implemented atomic `reserve_publish_slot(message_id, min_interval_seconds)` in `PipelineStateRepository` under immediate transaction locks. Checks both completed publications and active reservations across all other messages within the rate-limit window. If the slot is clear, it atomically reserves the slot with `publish: processing`. If locked, `runner.py` either sleeps asynchronously until the slot opens (`--wait-for-window`) or exits cleanly with `scheduled` status (exit code 0). Verified with comprehensive unit tests for concurrency contention.
