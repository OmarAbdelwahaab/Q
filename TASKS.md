...
- [x] **Partially fixed: original concurrent-trigger idempotency race resolved, but see new finding below.** Implemented atomic `claim_execution(message_id, stage, force)` in `PipelineStateRepository`. The narrow case this targeted — two concurrent triggers for the same still-in-flight message_id — is genuinely closed on SQLite via `BEGIN IMMEDIATE`. A `--force` CLI flag was added for manual re-runs.
- [ ] **New finding (reproduced): a "scheduled" outcome permanently deadlocks the message.** `claim_execution()` sets `orchestration: processing` at the top of `run()`, but every early-return path for `status="scheduled"` (posting window closed, or `reserve_publish_slot` reports the slot busy) with `wait_for_window=False` returns without releasing that claim. Reproduced directly: a message deferred once by a closed posting window comes back `skipped_active_execution` on every later retry — including after the window opens — and never actually renders or publishes unless manually rerun with `--force`. This defeats the "zero manual steps in the happy path" goal in SPEC §2 for anything the scheduler ever defers. Fix: release/reset the `orchestration` claim on every `scheduled` early return (e.g. `upsert_stage(message_id, "orchestration", "scheduled")`), not just on failure/completion.
- [ ] **New finding (reproduced): dangling rate-limit reservation blocks unrelated messages after a downstream failure.** `reserve_publish_slot()` marks a message's `publish` stage `processing` *before* render/publish run (step 6, ahead of steps 7–8). If render or publish then fails, nothing releases that reservation. Reproduced: message A's render fails after its slot was reserved, and a completely unrelated, QA-approved message B run immediately after is incorrectly told `scheduled` / "Rate limit active" for the next `min_interval_seconds`, purely because of A's abandoned reservation. Fix: reserve the slot immediately before the actual publish call (end of step 8) rather than before render, or explicitly clear/release the `publish` row on any failure between reservation and the real publish attempt.
- [ ] **Test coverage claim needs correction.** The "verified with comprehensive unit tests for concurrency contention" note refers to `test_claim_execution_lifecycle` / `test_reserve_publish_slot_lifecycle`, which are sequential, single-threaded calls against SQLite exercising the state-machine's API contract — not tests that use threads/processes to exercise real concurrent access. Neither of the two new findings above was catchable by them.
- [ ] **Unverified: Postgres backend concurrency guarantee.** `claim_execution`/`reserve_publish_slot` issue `BEGIN IMMEDIATE` on SQLite, which — because SQLite locks the whole database file — happens to correctly serialize the entire read-check-write sequence, including across different `message_id`s. On Postgres they only issue a plain `BEGIN` (default READ COMMITTED), which does not hold any lock across the initial `SELECT` and the later `INSERT ... ON CONFLICT DO UPDATE` — only the final write is contended, via the unique index. This is exactly the backend the multi-container n8n setup is meant to use, and the fix has not been exercised against a real Postgres instance under concurrent load. Worth verifying with `SELECT ... FOR UPDATE` on a lock row, or `pg_advisory_xact_lock()`, before trusting it in production.
- [x] **Fixed and confirmed: scheduler rate-limit race — narrow case resolved.** Implemented atomic `reserve_publish_slot(message_id, min_interval_seconds)`. Two different messages racing to reserve the *same* slot at the same instant is closed on SQLite; see the two new findings above for what's not yet resolved.
- [ ] **New finding (reproduced): `build_orchestrator()` crashes immediately, in every configuration, due to three constructor-signature mismatches.** Confirmed by actually calling the real (unmocked) `build_orchestrator()` — something no existing test does (see the next item). All three are kwarg-name drift between the call site in `pipeline/orchestration/app.py` and the callee's real `__init__`:
  1. `CtcForcedAligner(binary_path=..., model_name=...)` → the constructor takes `binary=`, `model=`. Fires first, unconditionally, on every single call regardless of environment.
  2. `CaptionTemplater(template=...)` → the constructor takes `default_template=`. Also unconditional.
  3. `MultiPlatformPublishClient(base_url=...)` → the constructor takes `api_base_url=`. Only fires when `PUBLISH_API_KEY` is set (real/production config, not the local `StubPublishClient` default) — reproduced by setting a fake key and re-running.

  Verified the complete fix by patching all three locally and re-running `build_orchestrator()` with a seeded corpus cache (to avoid a live network fetch) and a real `PUBLISH_API_KEY` set: it succeeds end-to-end only once all three are corrected, confirming this is the full set for this function. Fix priority: #1 must go first since it's the one that actually fires today; #2 and #3 are currently masked by it.
- [ ] **Confirmed gap: no test calls the real `build_orchestrator()`.** Traced every usage in the test suite: it's imported once, only to be `patch()`-mocked out in `test_cli_main_exit_code_mapping`. It is never invoked directly anywhere else, including in the "end-to-end dry run" integration test, which wires services manually instead of going through the factory. This is exactly how the three constructor-mismatch bugs above went undetected. Add a test that calls the real function with realistic/representative env vars (seeded corpus cache to avoid a live network fetch) and asserts it returns without raising.
- [ ] **New finding (confirmed): every `workflow.json` node after the first uses the wrong message_id expression.** Every downstream `executeCommand` node's command interpolates `{{$json["message"]["message_id"]}}`. In n8n, `$json` inside a node's expression refers to *that node's own input* — i.e. the immediately preceding node's output — not the original trigger payload. Since every node here is a shell command whose output is just its own stdout (not the Telegram message JSON), this expression only actually resolves on "Ingest Video Message" (directly downstream of the trigger). Every later node — Extract Audio WAV, Recognize Quran Verses, CTC Forced Alignment, QA Gate Verification, Render 1080x1920 Video, Multi-Platform Publish — needs an explicit named-node reference instead, e.g. `{{$('Telegram Video Trigger').item.json.message.message_id}}`.
- [ ] **New finding (confirmed): the posting-window/rate-limit check never actually gates anything in `workflow.json`.** Traced the node connection graph directly: `Posting Window & Rate Limit Check` flows straight into `Render 1080x1920 Video` unconditionally — no IF node in between, unlike the QA-gate branch, which correctly has one (`Check QA Gate Approved`). The node's command does compute and print `can_post` to stdout, but nothing in the workflow graph ever reads it. Needs an IF node after this step, mirroring the QA-gate pattern, that only proceeds to Render when `can_post` is true.
