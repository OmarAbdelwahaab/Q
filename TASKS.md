...
- [ ] **Code review follow-up: refactor `PipelineOrchestrator.run()` duplication.** The 6 stage blocks (audio, recognition, alignment, qa_gate, render, publish) repeat the same try/except → log → `upsert_stage(..., "failed", ...)` → alert → return-`PipelineExecutionSummary` shape almost verbatim, ~250 lines of near-identical boilerplate. Extract a shared `_run_stage(name, coro)` helper so failure handling only has to change in one place
- [ ] **Code review follow-up: dead import in `runner.py`.** `datetime, timezone` are imported at the top of the file but never used — remove
- [ ] **Code review follow-up: no connection pooling in `PipelineStateRepository`.** `_connect()` opens a brand-new DB connection on every single query. Fine for SQLite/local CLI use, but once multiple n8n worker containers are hitting the same Postgres instance concurrently (the scenario this dual-tier design is meant for) this adds real per-query latency — worth pooling (`psycopg_pool` or similar) before that happens
- [ ] **Code review follow-up: idempotency check-then-act race.** `run()`'s idempotency guard (read `publish` stage status → then execute all 8 stages) isn't atomic. Two concurrent invocations for the same `message_id` (e.g. a duplicate n8n trigger firing before the first run completes) could both pass the guard before either finishes, which would violate the "reprocessing the same message_id must never double-publish" requirement in SPEC §6. Needs a claim/lock step (e.g. an atomic "processing" status write guarded by the existing `(message_id, stage)` primary key, or `SELECT ... FOR UPDATE` on the Postgres backend) before stage 2 starts
- [ ] **Code review follow-up: scheduler rate-limit race.** `PostingWindowScheduler.evaluate()` reads `fetch_latest_published_timestamp()` and decides `can_post` without holding a lock, so two orchestrator instances evaluating near-simultaneously could both get a green light and both publish inside the same rate-limit window. Same root cause as the idempotency race above; likely fixable with the same locking mechanism
