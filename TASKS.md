...
- [ ] **New finding (confirmed, root cause verified): the posting-window/rate-limit check never actually gates anything in `workflow.json` — and adding an IF node alone would not fix it.** Traced the node connection graph directly: `Posting Window & Rate Limit Check` flows straight into `Render 1080x1920 Video` unconditionally. Checked why this differs from the QA-gate branch immediately before it, which correctly gates via `Check QA Gate Approved` (an IF node testing `$json["exitCode"] == 0`, fed by `qa_gate.app` explicitly returning `0`/`1`) — confirmed this is the workflow's own established convention (stage CLI signals pass/fail via exit code → IF node branches on it), and confirmed the posting-window check's one-liner does not follow it: it only does `print(d.can_post)` and always exits `0` regardless of the result, so even copying the QA-gate IF node verbatim would not gate anything. Also confirmed there is no backstop elsewhere: `pipeline/render/app.py` and `pipeline/publish/app.py` (what the next two nodes actually invoke) never reference `PostingWindowScheduler` at all, and the workflow's trigger is event-driven (`telegramTrigger`, fires on any new channel message) rather than cron-restricted to the posting window — so nothing else in the system prevents an off-hours or rate-limited post through this path. Fix needs two parts: (1) change the check's one-liner to `sys.exit(0 if d.can_post else 1)` so it can signal via exit code like every other gated stage, then (2) add an IF node after it — mirroring `Check QA Gate Approved` exactly — that only proceeds to Render when `exitCode == 0`.
